    ...
    <div class="form-group-item">
        ...
        <div class="input-group mb-3">
            <div class="input-group-prepend">
                <span
                    class="input-group-text">{{__('Credit')}} {{ !empty(auth()->user()) ? auth()->user()->balance : 0  }}</span>
            </div>
        ...
        <div class="d-flex justify-content-between"><h5 class="form-section-title">{{__("Pay now")}}:</h5>
            <div
                class="val convert_pay_now">{{format_money(floatval($booking->deposit == null ? $booking->total : $booking->deposit))}}</div>
        </div>
    </div>
@endif
